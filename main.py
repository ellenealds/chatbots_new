...
from new_adhd_config import new_adhd_config
import logging
...
@st.experimental_singleton
def config_adhd (new_adhd_config):
    co = cohere.Client("b55ru767vjHE7IuQ0aaqQx9rIO3g7CN0zquVnEZY")

    data_scientist_config = new_adhd_config
...
# aet the session state
adhd_bot = config_adhd(new_adhd_config)
...
page = st.sidebar.selectbox('Who would you like to speak to?', ['Data Scientist Specialist', 'Data Analyst Specialist', "ADHD Specialits"])
...
# add page for chatbot
elif page == 'ADHD Specialist':
    # Add a title to the page
    st.title('ADHD Specialist')
    # add a subheader explaining what the page is about
    st.subheader('Ask a question about any ADHD concepts')
    # this needs a chatbot interface
    # add a user input for the question
    question = st.text_input('Ask a question')
    # add a button to generate the answer
    if st.button('Submit'):
        # call the data_scientist_bot
        adhd_bot.reply(question)
        # get the response and set session state
        
        response = adhd_bot.chat_history
        # reverse the response so the most recent message is at the top
        response = response[::-1]
        # Generate the formatted response
        formatted_response = generate_response(response)
